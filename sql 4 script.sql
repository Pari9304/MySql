...
select region,sum(Quantity) as ProductsSold 
from Sales 
group by region;

#3. Write a query to get the average sales amount per product.
select ProductID, avg(quantity*UnitPrice) as AverageSale 
from Sales 
GROUP BY ProductID;
...
select CustomerID, count(Quantity) as SalesRows 
from Sales 
group by CustomerID;
...
select Region, avg(UnitPrice) as avgUnitPrice 
from Sales 
group by Region 
having avgUnitPrice>200;
...
select CustomerId, count(SaleID) as Purchases 
from Sales 
group by CustomerID 
having Purchases>2;
